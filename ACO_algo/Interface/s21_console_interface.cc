...
void ConsoleInterface::fileReading() {
  while (true) {
    std::cout << style3 << " please enter file name                      "
              << end << std::endl;
    std::cout << std::endl;
    std::string filename;
    std::cin >> filename;
...
void ConsoleInterface::saveToDot() {
  std::cout << "\n"
            << style3 << " please enter file name to save              " << end1
            << std::endl;
  std::string filename;
  std::cin >> filename;
  mainGraph.exportGraphToDot(filename);
  std::cout << "\n"
            << style3 << " file successfully saved in Graphs directory " << end1
            << std::endl;
  std::cerr << style4
...
void ConsoleInterface::AntColonyOptimization() {
  try {
    ...
    std::cout << std::endl;
    std::cout << style3 << " 1. ACO Algoritm in simply way               "
              << end << std::endl;
    printVector(res.first.vertices);
    std::cout << style4 << "DISTANCE: ";
    std::printf("%3.0f\n\n", res.first.distance);
    std::cout << style4 << "RUNTIME: ";
    std::printf("%8.5f sec\n\n", res.second);

    res = algorithms.solveTravelingSalesmanProblemMultithreads(mainGraph, N);
    std::cout << style3 << " 1. ACO Algoritm with Multithreads           "
              << end << std::endl;
    printVector(res.first.vertices);
    std::cout << style4 << "DISTANCE: ";
    std::printf("%3.0f\n\n", res.first.distance);
...
void ConsoleInterface::menuOrExit() {
  std::cout << style3 << " to return to the menu press 1               " << end
            << std::endl;
  std::cout << style3 << " to exit the program press 0                 " << end1
            << std::endl;
  int choice;
  std::cin >> choice;
  if (choice == 1) {
...
void ConsoleInterface::menuText() {
  std::cout << style1 << "---------------------------------------------" << end
            << std::endl;
  std::cout << style2 << "               ALGORITHMS MENU               " << end
            << std::endl;
  std::cout << style1 << "---------------------------------------------" << end1
            << std::endl;
  std::cout << style3 << " 1. Ant colony algorithm                     " << end
            << std::endl;
  std::cout << style3 << " 2. Save graph to .dot                       " << end
            << std::endl;
  std::cout << style3 << " 3. Upload new file                          " << end
            << std::endl;
