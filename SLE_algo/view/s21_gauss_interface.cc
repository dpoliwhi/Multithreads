...
void GaussInterface::informationPart() {
  std::cout << style1 << "\n---------------------------------------------" << end << std::endl;
  std::cout << style2 << "       WELCOME TO PROGRAM TO SOLVE SLE       " << end << std::endl;
...
void GaussInterface::endMenu() {
  std::cout << style1 << "---------------------------------------------" << end << std::endl;
  std::cout << style2 << "              ALGORITHM COMPLITED            " << end << std::endl;
  std::cout << style1 << "---------------------------------------------" << end1 << std::endl;
  std::cout << style3 << " 1. Restart program                          " << end << std::endl;
  std::cout << style3 << " 0. Exit from the program                    " << end1 << std::endl;
}

void GaussInterface::firstMenu() {
  SLE.clearData();
  std::cout << style3 << " 1. Load matrix from file                    " << end << std::endl;
  std::cout << style3 << " 2. Enter matrix                             " << end1 << std::endl;
  int choice;
  if (std::cin >> choice) {
    if (choice == 1) {
      fileReading();
...
void GaussInterface::fileReading() {
  while (true) {
    std::cout << "\n" << style3 << " Please enter file name                      " << end << std::endl;
    std::cout << std::endl;
    std::string filename;
    std::cin >> filename;
    try {
...
void GaussInterface::enterData() {
  std::cout << "\n" << style3 << " Enter matrix size                           " << end1 << std::endl;
  try {
    SLE.getSize();
  } catch (std::exception &exceptionText) {
    std::cerr << style4 << "\nError: " << exceptionText.what() << end << "\n\n";
  ...
  std::cout << "\n" << style3 << " Enter matrix values                         " << end1 << std::endl;
  SLE.getData();
  runTimer();
  exitOrRestart();
}
...
void GaussInterface::printMainData() {
  std::cout << "\n" << style3 << " Main Data                                   " << end1 << std::endl;
  SLE.printMatrix();
  std::cout << "\n" << end;
}
...
void GaussInterface::runTimer() {
  printMainData();
  int cycles;
  std::cout << "\n" << style3 << " Enter number of cycles                      " << end1 << std::endl;
  if (std::cin >> cycles) {
    try {
      Simpletimer timer;
      timer.startTime();
...
void GaussInterface::runUsualGauss(int cycles) {
  for (int i = 0; i < cycles; i++) {
    SLE.usualGauss();
    if (i == 0) {
      std::cout << "\n" << style3 << " Usual Gauss Answer:                         " << end1 << std::endl;
      SLE.printResult();
    }
  }
}
...
void GaussInterface::runParallelGauss(int cycles) {
  for (int i = 0; i < cycles; i++) {
    SLE.parallelGauss();
    if (i == 0) {
      std::cout << "\n" << style3 << " Parallel Gauss Answer:                         " << end1 << std::endl;
      SLE.printResult();
    }
  }
}
...
void GaussInterface::handleIncorrectMenuChoice() {
  ...
  std::cout << style4 << "\nError: incorrect input, try again " << end1 << std::endl;
}
